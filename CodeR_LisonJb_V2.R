...
# Table interm?diaire avec les bons types de variables
...
## TRAVAIL SUR LA REGION DU MONDE
# D?couverte de la variable
table(CoreTable_training$REGION_C, useNA="always")
# On peut la pr?duire par un multinomial non ordonn?
# Modalit? de r?f?rence : la plus repr?sent?e dans les donn?es i.e. WESTERN EUROPE
ml.region <- vglm(as.factor(REGION_C) ~ RACE_C+AGEGRP2+BMI+WEIGHTBL+HEIGHTBL, family=multinomial(refLevel=5), data=CoreTable_training)
summary(ml.region)
# Age tr?s peu significatif, on l'enl?ve : meilleure vraisemblance
ml.region <- vglm(as.factor(REGION_C) ~ RACE_C+BMI+WEIGHTBL+HEIGHTBL, family=multinomial(refLevel=5), data=CoreTable_training)
summary(ml.region)
# Tests
(q0 = head(predict(ml.region)))
q2 = predict(ml.region, newdata = head(CoreTable_training[is.na(REGION_C),]), tpe="terms")
all.equal(q1, q2) # Should be TRUE
# JE N'ARRIVE PAS A FAIRE LA PREDICTION !!!


## TRAVAIL SUR LES DONNEES BIOLOGIQUES
# Donn?es biologiques dont on peut compl?ter les valeurs manquantes :
# LDH, TESTO (tr?s importante), NA., MG, PHOS, ALB, TRPO, CCRC, GLU
# Plot des histogrammes des variables pour voir si elles ont une distribution sp?cifique

par(mfrow=c(1,1))
hist(CoreTable_training$LDH, breaks=100)
hist(CoreTable_training$TESTO, breaks=100)
hist(CoreTable_training$NA., breaks=100)
hist(CoreTable_training$MG, breaks=100)
hist(CoreTable_training$PHOS, breaks=100)
hist(CoreTable_training$ALB, breaks=100)
hist(CoreTable_training$TPRO, breaks=100)
hist(CoreTable_training$CCRC, breaks=100)
hist(CoreTable_training$GLU, breaks=100)

# Etude de ces donn?es par rapport ? DEATH pour voir si une ou plusieurs semblent corr?l?es ? DEATH
names.bio <- c("LDH", "TESTO", "NA.", "MG", "PHOS", "ALB", "TPRO", "CCRC", "GLU")
par(mfrow=c(3,3))
for (i in names.bio)
{
  plot(CoreTable_training[[i]], CoreTable_training$DEATH, ylab="DEATH", xlab=i)
}
for (i in names.bio)
{
  boxplot(CoreTable_training[[i]]~CoreTable_training$DEATH, data=CoreTable_training, xlab="Death", ylab=i)
}
# Dispersion qui semble diff?rente pour chaque modalit? de DEATH pour les variables : NA. PHOS, ALB, GLU
# Donc a priori n?cessaire de les compl?ter

# On va essayer de les compl?ter en utilisant une r?gression lin?aire sur les autre variables biologiques (o? le pourcentage de valeurs manquantes est faible)
attach(CoreTable_training)

# 1 - LDH
#----------
reg.LDH <- lm(LDH~ALP+ALT+AST+CA+CREAT+HB+NEU+PLT+PSA+TBILI+WBC)
summary(reg.LDH)
# On garde ALP, ALT, AST, CREAT, HB et TBILI et on compl?te par la pr?vision
reg.LDH <- lm(LDH~ALP+ALT+AST+CREAT+HB+NEU+TBILI)
summary(reg.LDH)
# Le R2 n'est toujours pas tr?s bon mais les variables sont significatives
# On pr?dit avec ce mod?le
beta.LDH <- summary(reg.LDH)$coefficients[,1]
# On ne peut pr?dire que pour le ?l?ments o? toutes les covariables sont pr?sentes
# Reste un tout petit pourcentage qui reste manquant
names.X.LDH <- c("ALP","ALT","AST","CREAT","HB","NEU","TBILI")
LDH.to.complete <- CoreTable_training[is.na(CoreTable_training$LDH)==TRUE,names.X.LDH]
for (i in names.X.LDH)
{
  LDH.to.complete <- LDH.to.complete[!is.na(LDH.to.complete[[i]]),]
}
# Pr?diction
LDH.to.inject <- as.matrix(cbind(1,LDH.to.complete))%*%t(t(beta.LDH))
# Remplacement
CoreTable_training$LDH[as.numeric(row.names(LDH.to.inject))] <- LDH.to.inject
# V?rification de la distribution obtenue
par(mfrow=c(1,1))
hist(CoreTable_training_typeOK$LDH, breaks=100)
hist(CoreTable_training$LDH, breaks=100)
CHECK_NA(CoreTable_training)
...
# 2 - TESTO
#----------
reg.TESTO <- lm(TESTO~ALP+ALT+AST+CA+CREAT+HB+NEU+PLT+PSA+TBILI+WBC)
summary(reg.TESTO)
# On garde CA et PLT
reg.TESTO <- lm(TESTO~CA+PLT)
summary(reg.TESTO)
# R2 vraiment beaucoup trop bas
# V?rification de la forme du nuage de points avec ?cr?tage des valeurs extr?mes
test <- CoreTable_training[(TESTO<=1.5),c("TESTO","CA","PLT")]
plot(test[,1],test[,2])
plot(test[,1],test[,3])
# C'est lin?aire mais en dehors des variables extr?mes
test2 <- CoreTable_training[(TESTO>1.5),c("TESTO","CA","PLT")]
plot(test2[,1],test2[,2])
plot(test2[,1],test2[,3])
# Sorte de deux mod?les lin?aires accol?s : TRES DUR A PREDIRE ...

# A VOIR POUR LES AUTRES VARIABLES
